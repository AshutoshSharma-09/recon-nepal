...
import { StockAcqUploadModal } from "./StockAcqUploadModal";
...
export interface SmaReconRow {
    match_id: number | null;
    match_uid: string | null;
    portfolio_id: string | null;
    scrip: string | null;
    stock_name: string | null;
    acq_qty_sum: number | null;
    th_credit_qty_sum: number | null;
    status: "MATCHED" | "UNMATCHED" | "EXCEPTION";
    match_kind: string | null;
    match_action: string | null;
    reason: string | null;
    finding_id: number | null;
    acq_entry_ids: string | null;
    th_entry_ids: string | null;
}

interface StockAcquisitionReconProps {
    rows: SmaReconRow[];
    batchId: number | null;
...
function simplifyReason(raw: string | null): string {
    ...
    // Auto-match success pattern: SUM(Qty)=X == SUM(Credit_Quantity)=X
    ...
    // Missing credit quantity
    if (/missing.*credit_quantity/i.test(raw) || /credit_quantity.*missing/i.test(raw)) {
        return "No received quantity found in Transaction History for this stock";
    }

    // Missing qty
    if (/missing.*qty/i.test(raw) || /qty.*missing/i.test(raw)) {
        return "No acquisition quantity found for this stock";
    }
    ...
    if (/no.*acquisition/i.test(raw) || /acquisition.*not found/i.test(raw)) {
        return "No acquisition record found for this stock";
    }

    // Quantity mismatch
    if (/sum.*qty.*!= |sum.*credit.*!=/i.test(raw) || /qty.*mismatch/i.test(raw)) {
        return "Acquisition quantity does not match the received quantity in Transaction History";
    }
...
export default function StockAcquisitionRecon({
    rows,
    batchId,
    onAcquisitionUpload,
    onHistoryUpload,
    ...
    const [breakTargetRow, setBreakTargetRow] = useState<SmaReconRow | null>(null);
    ...
    const handleBreak = async () => {
        ...
        try {
            ...
            const url = isMatchRow
                ? `${API_URL}/api/v1/sma-recon/match/break`
                : `${API_URL}/api/v1/sma-recon/finding/break`;
    ...
    const handleConfirmLink = async () => {
        ...
        try {
            const res = await fetch(`${API_URL}/api/v1/sma-recon/finding/link`, {
                method: "POST",
                headers: makeHeaders(),
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 flex flex-col">
            ...
            {showUpload && (
                <StockAcqUploadModal
                    ...
                    hasAcquisitionFile={hasAcquisitionFile}
                    hasHistoryFile={hasHistoryFile}
                    onAcquisitionUpload={onAcquisitionUpload}
                    onHistoryUpload={onHistoryUpload}
                    isAcquisitionUploading={isAcquisitionUploading}
                    ...
                    <table className="w-full text-left border-collapse text-xs table-fixed">
                        ...
                        <thead className="bg-slate-50 border-b border-slate-200 sticky top-0">
                            <tr>
                                ...
                                <th className="px-3 py-2.5 text-slate-600 font-semibold text-right whitespace-nowrap">Acq Qty Sum</th>
                                <th className="px-3 py-2.5 text-slate-600 font-semibold text-right whitespace-nowrap">MERO Credit Qty</th>
                                    ...
                                    return (
                                        <tr key={rowKey} className="hover:bg-slate-50 transition-colors">
                                            ...
                                            <td className="px-3 py-2.5 font-mono text-slate-700 whitespace-nowrap">
                                                {row.portfolio_id ?? <span className="text-gray-300">—</span>}
                                            </td>
                                            ...
                                            {/* Stock Name */}
                                            <td className="px-3 py-2.5 text-slate-600 max-w-[160px] truncate" title={row.stock_name ?? ""}>
                                                {row.stock_name ?? <span className="text-gray-300">—</span>}
                                            </td>

                                            {/* Acq Qty Sum */}
                                            <td className="px-3 py-2.5 text-right font-mono text-slate-700">
                                                {fmtNum(row.acq_qty_sum)}
                                            </td>

                                            {/* TH Credit Qty */}
                                            <td className="px-3 py-2.5 text-right font-mono text-slate-700">
                                                {fmtNum(row.th_credit_qty_sum)}
                                            </td>
