...
export function StockUploadModal({
  onClose,
  onAutoMatch,
  isProcessing,
  hasSummaryFile,
  ...
  const handleSummaryChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    if (!file.name.toLowerCase().endsWith(".csv")) {
      setError("Only CSV files are accepted for Stock Summary.");
      return;
    }
    setError("");
    setSummaryName(file.name);
    await onSummaryUpload(file);
  };
  ...
  const handleAutoRecon = async () => {
    if (!hasSummaryFile || !hasHistoryFile) {
  ...
  const isUploading = isSummaryUploading || isHistoryUploading;
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md mx-4 overflow-hidden">
        ...
        <div className="p-6 space-y-5">
          ...
          <div>
            ...
            <button
              ...
              className={`w-full flex items-center gap-3 border-2 border-dashed rounded-lg p-4 transition-all text-left
                ...
                } ${isSummaryUploading ? "opacity-60 cursor-wait" : "cursor-pointer"}`}
            >
              {isSummaryUploading ? (
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs font-medium truncate">
                  {isSummaryUploading ? "Uploading…" : summaryName || "Click to select Stock Summary CSV"}
                </p>
                {!summaryName && !isSummaryUploading && (
                  <p className="text-xs text-slate-400 mt-0.5">Expected: Portfolio_ID, Symbol, Stock_Name, Qty</p>
                )}
              </div>
            </button>
            <input
          ...
          <div>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs font-medium truncate">
                  {isHistoryUploading ? "Uploading…" : historyName || "Click to select Transaction History CSV"}
                </p>
                {!historyName && !isHistoryUploading && (
                  <p className="text-xs text-slate-400 mt-0.5">Expected: Portfolio_ID, Scrip, Transaction_Date, Balance_After_Transaction</p>
                )}
              </div>
            </button>
            <input
        ...
        <div className="px-6 pb-6 flex gap-3">
          <button
            ...
            disabled={isProcessing || isUploading || !hasSummaryFile || !hasHistoryFile}
