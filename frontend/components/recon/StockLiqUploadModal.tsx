...
interface StockLiqUploadModalProps {
    onClose: () => void;
...
export function StockLiqUploadModal({
    onClose,
    onAutoMatch,
    isProcessing,
    hasLiquidationFile,
    ...
    isHistoryUploading,
}: StockLiqUploadModalProps) {
    const liqRef = useRef<HTMLInputElement>(null);
    ...
    const handleLiqChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        const file = e.target.files?.[0];
        if (!file) return;
        if (!file.name.toLowerCase().endsWith(".csv")) {
            setError("Only CSV files are accepted for Stock Liquidations.");
            return;
        }
        setError("");
        setLiqName(file.name);
        await onLiquidationUpload(file);
    };
    ...
    const handleAutoRecon = async () => {
        if (!hasLiquidationFile || !hasHistoryFile) {
    ...
    const isUploading = isLiquidationUploading || isHistoryUploading;
            ...
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-md mx-4 overflow-hidden">
                ...
                <div className="bg-gradient-to-r from-[#172554] to-[#1e3a8a] px-6 py-4 flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-blue-200 text-xs mt-0.5">Stock Liquidation Reconciliation</p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="p-6 space-y-5">
                    ...
                    {/* Stock Liquidation CSV Upload */}
                    <div>
                        <label className="block text-xs font-semibold text-slate-600 uppercase tracking-wide mb-2">
                            Stock Liquidations CSV
                            <span className="text-red-500 ml-1">*</span>
                        </label>
                        <button
                            type="button"
                            onClick={() => liqRef.current?.click()}
                            disabled={isLiquidationUploading}
                ...
                ${hasLiquidationFile
                                ...
                                } ${isLiquidationUploading ? "opacity-60 cursor-wait" : "cursor-pointer"}`}
                        >
                            {isLiquidationUploading ? (
                            ...
                            <div className="flex-1 min-w-0">
                                <p className="text-xs font-medium truncate">
                                    {isLiquidationUploading ? "Uploading…" : liqName || "Click to select Stock Liquidations CSV"}
                                </p>
                                {!liqName && !isLiquidationUploading && (
                                    <p className="text-xs text-slate-400 mt-0.5">Expected: Portfolio_ID, Scrip, Qty</p>
                                )}
                            </div>
                        </button>
                        <input ref={liqRef} type="file" accept=".csv" className="hidden" onChange={handleLiqChange} />
                    </div>
                        ...
                        >
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                {!histName && !isHistoryUploading && (
                                    <p className="text-xs text-slate-400 mt-0.5">Expected: Portfolio_ID, Scrip, Transaction_Date, Debit_Quantity</p>
                                )}
                            </div>
                        </button>
                ...
                <div className="px-6 pb-6 flex gap-3">
                    <button
                        ...
                        disabled={isProcessing || isUploading || !hasLiquidationFile || !hasHistoryFile}
